...
public class Manager extends Handler {
    ...
    private final Queue<EffectToast> notifyQueue;

    private boolean isSticky=false;

    private Manager() {
        notifyQueue = new LinkedBlockingQueue<EffectToast>();
    }
    ...
    void add(EffectToast crouton,boolean repeat ) {
    ...
    void addSticky(EffectToast crouton) {
    ...
    void removeSticky(){
        EffectToast notify = notifyQueue.peek();
        if(notify!=null) {
    ...
    private long calculateCroutonDuration(EffectToast notify) {
    ...
    private void sendMessage(EffectToast crouton, final int messageId) {
        final Message message = obtainMessage(messageId);
        message.obj = crouton;
        sendMessage(message);
    }

    private void sendMessageDelayed(EffectToast crouton, final int messageId, final long delay) {
    ...
    public void handleMessage(Message msg) {
        final EffectToast notify = (EffectToast) msg.obj;
        if (null == notify) {
            return;
        }
        switch (msg.what) {
    ...
    private void displayNotify(boolean sticky) {
        ...
        final EffectToast currentNotify = notifyQueue.peek();
    ...
    private void addNotifyToView(final EffectToast notify) {
        if (notify.isShowing()) {
            return;
        }
    ...
    protected void removeNotify(EffectToast notify) {
    ...
    protected void removeNotifyView(EffectToast notify) {
        View notifyView = notify.getView();
        ViewGroup notifyParentView = (ViewGroup) notifyView.getParent();
        if (null != notifyParentView) {

            EffectToast removed = notifyQueue.poll();
